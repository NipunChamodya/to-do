...
function App() {
  ...
  const splashTexts = ["I'm Nipun", "I made this", "with my inner peace"];

  // --- NEW: Splash Screen Logic ---
  useEffect(() => {
    if (!showSplash) return; // Don't run if splash is hidden

    if (splashTextIndex < splashTexts.length) {
      const timer = setTimeout(() => {
        setSplashTextIndex(splashTextIndex + 1);
      }, 1800); // Time each message is shown
      return () => clearTimeout(timer);
    } else {
      const finalTimer = setTimeout(() => {
        setShowSplash(false);
      }, 1500); // Time after the last message before fading out
      return () => clearTimeout(finalTimer);
    }
  }, [splashTextIndex, showSplash]);
